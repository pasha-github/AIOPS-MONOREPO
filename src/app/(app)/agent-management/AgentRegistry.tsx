...
export default function AgentRegistry({
  ...
  const openWebhookModal = (agent: AgentRecord) => {
  ...
  useEffect(() => {
    if (!webhookTarget || webhookTab !== "list") {
      return;
    }

    const controller = new AbortController();
    const loadWebhooks = async () => {
      const agentId = webhookTarget.agent_id?.trim();
      if (!agentId) {
        ...
        setWebhooks(
          data
            .map((item) => {
              ...
              const webhookId = typeof record.webhook_id === "string" ? record.webhook_id.trim() : "";
  ...
  }, [agentManagerBaseUrl, webhookTab, webhookTarget]);

  const handleCreateWebhook = async () => {
    if (!webhookTarget || isCreatingWebhook) {
      return;
    }
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setWebhookDeleteTarget({
                                    agentId: webhookTarget.agent_id?.trim() ?? "",
