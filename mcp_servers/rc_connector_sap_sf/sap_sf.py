...
HEADERS_UPDATE = {
    ...
    "Accept": "application/json",
    "Content-Type": "application/json",
    "X-HTTP-Method": "MERGE",  # Standard SAP OData v2 partial update
}
...
def get_status_list():
    url = f"{BASE_URL}/JobApplicationStatusLabel?$select=appStatusId,statusLabel,locale&$format=json"
    try:
        r = requests.get(url, headers=HEADERS, timeout=10)
        if r.status_code != 200:
            return []
...
def get_status_map():
    ...
    for s in statuses:
        status_id = str(s.get("appStatusId", "")).strip()
...
def build_preview_html() -> str:
    ...
    <style>
        ...
        .sap-panel {
            border: 1px solid #7192b9;
            background-color: #ffffff;
            margin-bottom: 12px;
        }
        .sap-panel-header {
            background: linear-gradient(to bottom, #3b6c9e 0%, #1d4d7a 100%);
            color: #ffffff;
            font-weight: bold;
            padding: 6px 10px;
            font-size: 12px;
...
/* Classic SAP NetWeaver Panel Boxes */
.sap-panel {
    border: 1px solid #7192b9;
    background-color: #ffffff;
    margin-bottom: 12px;
...
.sap-tabs {
    ...
    border-bottom: 1px solid #7192b9;
    padding-left: 5px;
    padding-top: 3px;
}
...
table.sap-table td input[type="text"] {
    width: 100%;
...
button.sap-btn {
    ...
    background: linear-gradient(to bottom, #ffffff 0%, #e0e9f2 100%);
    color: #000000;
    padding: 2px 10px;
    font-size: 11px;
    cursor: pointer;
...
"""
    ...
    <!-- Right Column: Sidebar Form & Status -->
    <div class="right-col">
                ...
                <form id="updateForm" method="post" action="/update" onsubmit="handleFormSubmit(event)">
                    ...
                    <div class="form-group">
                        ...
                        <select id="form_status" name="status">
                            <option value="14">14 - Hirable</option>
                            <option value="15">15 - Hired</option>
                        </select>
                    </div>
...
async function handleFormSubmit(event) {
    ...
    // Disable button & show on-screen text
