"""
Teams Beta Connector v1.0.0
Async version of the Teams connector — uses httpx.AsyncClient instead of
blocking requests so it does not stall the agent's event loop during I/O.
"""
...
class TeamsBetaConnector(BaseConnector):
    """
    Async connector for the Teams bot proactive alert APIs.
    ...
    async def _post_alert(
        ...
        """Send an authenticated alert request to the Teams bot API (async)."""
    ...
    @connector_tool
    async def send_alert_by_conversation(
        self, content: str, tool_context: ToolContext | None = None
    ) -> dict[str, Any]:
