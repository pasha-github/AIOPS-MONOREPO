...
// The agent sends plain text or Markdown -- never HTML directly. This is what makes the
// tool reliable regardless of how the agent formats its output: there's no way for the
// agent to send malformed/escaped HTML, because it never writes HTML at all. We own the
// HTML generation and guarantee it's valid.
...
async function sendMail({ to, subject, body, attachmentPaths }) {
  ...
  const message = {
    subject,
    body: { contentType: 'HTML', content: renderBody(body) },
