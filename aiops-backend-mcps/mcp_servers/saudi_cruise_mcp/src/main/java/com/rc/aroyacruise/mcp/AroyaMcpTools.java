...
public class AroyaMcpTools {
    ...
    public String getAvailableVoyages(
            @McpToolParam(description = "Start date range from, format yyyy-MM-dd", required = true) String startDateFrom,
            @McpToolParam(description = "Start date range to, format yyyy-MM-dd", required = true) String startDateTo,
            @McpToolParam(description = "List of destination codes, for example [\"Red\", \"GUL\"]", required = true) List<String> destinations,
            @McpToolParam(description = "List of departure port codes, for example [\"SAJED\", \"AEDXB\"]", required = true) List<String> departurePorts
    ) {
        return voyageService.getAvailableVoyagesMD(
    ...
    public JsonNode updateReservationAddons(
            ...
            @McpToolParam(
                    description = """
                        ...
                        - dateFrom: addon start date. Example: "2026-01-01T00:00:00"
                        - dateTo: addon end date. Example: "2999-01-01T00:00:00"
    ...
    @McpTool(
            name = "aroya_store_reservation",
            description = """
                Store/save an existing Aroya reservation.

                Use this tool after creating or updating a reservation.

                This calls reservationStore and returns the complete JSON response from Aroya.
                """
    )
    public JsonNode storeReservation(
        ...
        return reservationStoreService.storeReservation(
                new NodeReservationStoreRequest(
                        reservationId,
                        false
                )
        );
